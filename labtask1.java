class SY2022bit001{
public int getValidRegistrationsCountLe50(String []number){
	int count =0;
		...
		if(number[i] == null || number[i].length() != 10){
			...
			for(int j=0;j<number[i].length();j++){
				...
				if(j >= 4 && j < 7){
					...
					String LE =number[i].substring(7,10);
		
					if(character.getNumericvalue(LE.[charAt(2))==0 && character.getNumericvalue(LE.[charAt(1))<=5){
					count++;
...
public static void main(String[] args){
	...
	int count  =data.getValidRegistrationsCountLe50(RegNum);
