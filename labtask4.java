class SY2022bit001{
public int getValidRegistrationsCountTask4(String []number){
	int count =0;
	...
	for(int i=0;i<number.length;i++){
		...
		if(number[i] == null || number[i].length() != 10){
			...
			for(int j=0;j<number[i].length();j++){
				...
				if(j >= 4 && j < 7){
					...
					if(departmentId.equalsIgnoreCase("bit") || departmentId.equalsIgnoreCase("bcs") || departmentId.equalsIgnoreCase("bme") || departmentId.equalsIgnoreCase("bch")){
						...
						String max="050";
						int a=max.codePointAt(2);
						int b=max.codePointAt(1);
						int c=max.codePointAt(2);
						int d=max.codePointAt(1);
						if(f==a&&d<=b){
							count++
						}
			...
			long eclapsed=end-startTime;
			System.out.println("time:"+eclapsedTime+"nanoseconds");
			return count;
		}
	}	
}
